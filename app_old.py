"""
app.py — RetailBrain AI  (Professional Retail & E-commerce Analytics)
...
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="RetailBrain AI — Analytics",
    page_icon="🧠",
    layout="wide",
...
with st.sidebar:
    ...
    st.caption("Required columns: date, product, price, cost, quantity, customer")
...
if uploaded:
    source = uploaded
elif st.session_state.get("_main_upload"):
    source = st.session_state.pop("_main_upload")
...
if source is not None:
    try:
        ...
        if not ok:
            ...
            # Track dataset upload
            if current_user:
                filename = getattr(source, 'name', 'uploaded.csv')
                db.log_dataset(current_user["id"], filename, len(df), source.seek(0, 2) or 0)
                source.seek(0)
...
if st.session_state["df"] is None:
    st.markdown("""
    <div class="hero">
        <div class="hero-badge">AI-Powered Retail & E-commerce Analytics</div>
        <div class="hero-title">Smarter Decisions With<br><span>RetailBrain AI</span></div>
        <div class="hero-sub">
            Upload your sales data to unlock revenue insights, profit analysis,
            top-performing products, customer intelligence, and AI-powered
            recommendations for your retail business.
        </div>
    ...
    with col2:
        ...
        main_uploaded = st.file_uploader(
            ...
            type=["csv"],
            key="main_uploader",
            help="Upload your retail/e-commerce sales data CSV file"
        )
        if main_uploaded:
            st.session_state["_main_upload"] = main_uploaded
            st.rerun()

        st.markdown("**Or use the sidebar to upload from any page.**")
    ...
    st.markdown("""
        <div class="feature-grid" style="margin-top:2rem">
            <div class="feature-card">
                ...
                <div class="feature-desc">Monthly breakdowns with margin tracking and MoM analysis</div>
            </div>
            <div class="feature-card">
                <div class="feature-icon">🏆</div>
                <div class="feature-name">Product & Customer Intel</div>
                <div class="feature-desc">Identify top performers, margins, and growth opportunities</div>
            </div>
            <div class="feature-card">
                <div class="feature-icon">🤖</div>
                <div class="feature-name">AI Business Advisor</div>
                <div class="feature-desc">Ask questions in plain English and get data-driven answers</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
    st.stop()
...
<div class="topbar">
    <div class="topbar-logo">
        ...
        <div>
            <div class="topbar-title">RetailBrain AI</div>
            <div class="topbar-subtitle">Retail & E-commerce Analytics</div>
        </div>
    </div>
...
if auth.is_admin(current_user):
    tab_dash, tab_prod, tab_cust, tab_data, tab_chat, tab_profile, tab_admin = st.tabs([
        "  📈  Overview  ",
        "  🏆  Products  ",
        "  👥  Customers  ",
        "  🗃  Raw Data  ",
    ...
    tab_dash, tab_prod, tab_cust, tab_data, tab_chat, tab_profile = st.tabs([
        "  📈  Overview  ",
        "  🏆  Products  ",
        "  👥  Customers  ",
        "  🗃  Raw Data  ",
...
with tab_chat:
    ...
    if st.session_state.get("_pending_q"):
        ...
        try:
            ...
            if current_user:
                db.log_ai_request(current_user["id"], assistant.provider or "rule-based", _pq)
